...
var routing_log_path = flag.String("routing_log_path", "", "Routing decision log path")
...
func main() {
	...
	routing_log, err := os.Create(*routing_log_path)
	if err != nil {
		log.Fatal("Error creating routing log: %v", err)
	}
	route_logger := node.NewLogger(routing_log)
